...
nreplicas = 100;   % number of replicas syntethically generated
...
use_noise_ref = 0;
calc_noise_BW = 1;
noise_BW_correction = 1;   % 1 --> correct noise statistics for the system noise bandwidth
...
compute_g_factor = 0;      % 1 --> compute geometry factor for parallel imaging
...
acc = [1 4];                 % 2D acceleration factor [Freq Phase] ** only integer values **
...
homedir = './';
datadir = ['/home/montie01/Dropbox/For_Eros/SNR_rawdata_examples/Shoulder16ch_small_SNRRawData/'];
...
noise_file = [datadir 'meas_MID250_SNR_Noi_BW300_FID13608.dat'];
data_file  = [datadir 'meas_MID254_SNR_Sag_BW300_FID13612.dat'];
...
if nchan == 1 % body coil or single channel case
    ...
    if plot_reconstructed_image
        ...
        %         end
        title('Single Channel or Body Coil');
    end
    ...
    switch recon_method
        ...
        case 'opt'
            ...
            if use_pseudo_mr
                ...
                disp('Pseudo multiple replicas finished');
            else
                if simple_sens

...
for iacc = 1:length(acc_set_1D)
    ...
    if plot_reconstructed_image
        ...
        imagetitle = ['SENSE Reconstructed Image - Acc = ' num2str(R) ' X 1 (Coronal)'];
        figure
    ...
    if plot_g_factor
        % Display g-Factors
        gtitle = ['g-Factor - Acc = ' num2str(R) ' X 1 (Coronal)'];
        figure
        set(gcf,'Name',gtitle)
        colormap('parula');
        ...
        % Display 1/g-Factors
        invgtitle = ['Inverse g-Factor - Acc = ' num2str(R) ' X 1 (Coronal)'];
